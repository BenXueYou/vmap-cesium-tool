...
import { CesiumMapToolbarAdapter } from "./core/adapters/CesiumMapToolbarAdapter";
import type { LegacyToolbarConfig } from "./core/adapters";
...
let mapToolbar: CesiumMapToolbarAdapter | null = null;
...
onMounted(async () => {
  ...
  if (container) {
    ...
    // 创建适配器实例 - 暂时不传递回调，适配器内部会处理
    mapToolbar = new CesiumMapToolbarAdapter(
      viewer.value,
      container,
      toolbarConfig as LegacyToolbarConfig,
      undefined, // 不传递回调，使用适配器默认行为
      mapInitialCenter
    );
    if (mapToolbar) {
...
const customToolBarBtn = (mapToolbar: CesiumMapToolbarAdapter) => {
