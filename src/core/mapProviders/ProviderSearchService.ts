...
export function normalizeMapAuth(mapAuth?: MapAuthConfig): MapAuthConfig | undefined {
  if (!mapAuth) return undefined;
  return {
    tdt: mapAuth.tdt ? { token: clean(mapAuth.tdt.token), sk: clean(mapAuth.tdt.sk) } : undefined,
    gaode: mapAuth.gaode ? { key: clean(mapAuth.gaode.key), securityKey: clean(mapAuth.gaode.securityKey) } : undefined,
    tencent: mapAuth.tencent ? { key: clean(mapAuth.tencent.key) } : undefined,
    baidu: mapAuth.baidu ? { ak: clean(mapAuth.baidu.ak) } : undefined,
    google: mapAuth.google ? { apiKey: clean(mapAuth.google.apiKey), mapId: clean(mapAuth.google.mapId) } : undefined,
  };
}
...
export class ProviderSearchService {
  ...
  private buildUrl(endpoint: string, query: string, region: string, provider: BaseMapProviderId, auth?: MapAuthConfig) {
    let params: Record<string, string>;
    if (provider === 'tdt') {
      const payload = { start: 0, count: 10, queryType: 7, keyWord: query, mapBound: '73.5577,18.1597,135.0882,53.5609', level: 15 };
      params = { postStr: JSON.stringify(payload), type: 'query', tk: clean(auth?.tdt?.token) };
      if (auth?.tdt?.sk) params.sk = auth.tdt.sk;
    } else if (provider === 'gaode') {
      params = { city: region, key: clean(auth?.gaode?.key), keywords: query };
      if (auth?.gaode?.securityKey) params.sig = createAmapSignature(params, auth.gaode.securityKey);
    } else if (provider === 'baidu') {
      params = { query, region, output: 'json', ak: clean(auth?.baidu?.ak) };
    } else if (provider === 'tencent') {
      params = { boundary: `region(${region},0)`, keyword: query, key: clean(auth?.tencent?.key) };
    } else {
      params = { address: query, key: clean(auth?.google?.apiKey) };
    }
