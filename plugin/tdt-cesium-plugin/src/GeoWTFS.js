...
function combine(object1, object2, deep) {
    ...
    allKeys.forEach(key => {
        ...
        if (deep && typeof value1 === 'object' && typeof value2 === 'object' && value1 !== null && value2 !== null) {
            result[key] = combine(value1, value2, true)
        } else if (defined(value2)) {
            result[key] = value2
        } else {
            result[key] = value1
        }
...
class GeoWTFS {
    constructor(viewer, options) {
        if (!defined(viewer)) throw new Cesium.DeveloperError('viewer is required.');
        if(!defined(options.url)) throw new Cesium.DeveloperError('options.url is required.');
        ...
        const guid = crypto.randomUUID ? crypto.randomUUID() : Date.now().toString() + Math.random().toString(36).substr(2, 9);
    ...
    getTilesToRender(){
        const surface = this.viewer.scene.globe._surface;
        if (!defined(surface) || !defined(surface._tilesToRender) || surface._tilesToRender.length === 0) {
            return [];
        }
        const tiles =  surface._tilesToRender.map(function (e) {
            return {
                x: e.x,
                y: e.y,
                level: e.level,
    ...
    compareArray(tiles, latelyGrid){
        if (!defined(tiles) || !defined(latelyGrid) || !Array.isArray(tiles) || !Array.isArray(latelyGrid)) {
            return false;
        }
        if (tiles.length !== latelyGrid.length) {
            return false;
        }
        let flag = false;
    ...
    _queueCall(tiles){
        if (!defined(tiles) || !Array.isArray(tiles) || tiles.length === 0) {
            return;
        }
    ...
    createLabel(poi, tile){
        if(poi) {
            const entityOption = {
                show: true,
                position: Cesium.Cartesian3.fromDegrees(...poi.coordinate),
                label: {text: poi.name}
            }
            ...
            if(this.serverFirstStyle){
                ...
                if(undefined !== poi.fontColor){
                    entityOption.label.fillColor = Cesium.Color.fromCssColorString(this.HexadecimalConversion(poi.fontColor));
                }
                if(undefined !== poi.shiningColor){
                    entityOption.label.outlineColor = Cesium.Color.fromCssColorString(this.HexadecimalConversion(poi.shiningColor));
                }
            ...
            if(tile.t) {
                entityOption.label.verticalOrigin = Cesium.VerticalOrigin.CENTER;
                entityOption.label.horizontalOrigin = Cesium.HorizontalOrigin.CENTER;
                entityOption.billboard.verticalOrigin = Cesium.VerticalOrigin.CENTER;
                entityOption.billboard.horizontalOrigin = Cesium.HorizontalOrigin.CENTER;
            }
            const entity = new Cesium.Entity(entityOption);
