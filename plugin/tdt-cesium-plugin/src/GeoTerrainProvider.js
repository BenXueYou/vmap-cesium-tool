...
function f(e, x, d, t, _, h) {
    ...
    return defined(Cesium.Rectangle.intersection(i, x, h));
}

function createAvailability(provider) {
    const overallAvailability = [[[0, 0, 1, 0]]]
    const length = overallAvailability.length;
    const availability = new Cesium.TileAvailability(provider.tilingScheme, 19);
...
class GeoTerrainProvider extends Cesium.CustomHeightmapTerrainProvider {
    constructor(options) {
    ...
    requestTileGeometry(x, y, level, request) {
        ...
        const tileResource = Cesium.Resource.fetchArrayBuffer({url, request});
